...
Recommends questions to students based on their personal weakness profile.
"""
...
def recommend(student_matrix: np.ndarray, question_matrix: np.ndarray, 
    ...
    # FIXED: Use the student's personal weakness profile
    # The buggy version was using cohort_baseline and overwriting the student_profile
    
    # We want to target topics where the student is weak
    student_profile = student_matrix[student_idx]
    
    # Optional: We could emphasize gaps relative to cohort, but the core 
    # fix is to actually use the student's vector.
    # cohort_baseline = student_matrix.mean(axis=0)
    # student_profile = student_matrix[student_idx] - cohort_baseline
    # student_profile = np.maximum(student_profile, 0) # Only focus on where they are weaker than average

    # Normalize the profile to unit length
